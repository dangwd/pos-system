...
> **Cập nhật mới nhất (2026-06-16)**: Response detail (`GET /:id`) và list (`GET /`) của `ExchangeCurrency` nay trả thêm 5 field: `foreignAmount`, `targetCurrency`, `targetAmount`, `exchangeRate`, `targetRateToLak`. `foreignAmount` được lưu trực tiếp vào DB — không cần tính ngược từ `totalAmount / exchangeRate`.
> **2026-06-14**: Bổ sung mục [Phí gia công & Phí đá](#2-phí-gia-công--phí-đá) mô tả chi tiết `laborFee` / `stoneFee` cho mặt hàng vàng, bạc.
...
**Với giao dịch `ExchangeCurrency` (type = 7)**, response có thêm các field ngoại tệ:

```jsonc
{
  "id": "uuid",
  "invoiceCode": "NT-20260616-0001",
  "type": 7,
  "status": 2,
  "branchId": "uuid",
  "counterId": "uuid",
  "subtotalAmount": 2150000,   // = foreignAmount × exchangeRate (LAK)
  "laborFee": 0,
  "stoneFee": 0,
  "totalAmount": 2150000,
  "currency": "USD",           // loại tiền nguồn khách đưa
  "exchangeRate": 21500,       // tỷ giá nguồn → LAK
  "foreignAmount": 100,        // ← số tiền nguồn gốc khách đưa (lưu trực tiếp)
  "targetCurrency": "THB",     // loại tiền trả khách ("LAK" nếu không đổi chéo)
  "targetRateToLak": 600,      // tỷ giá đích → LAK (1 nếu đích = LAK)
  "targetAmount": 3583.3333,   // số tiền trả khách
  "paymentMethod": "CASH",
  "cashAmount": null,
  "bankAmount": null,
  "transactedAt": "2026-06-16T08:00:00Z",
  "items": []
}
```
  ...
  "data": [
    {
      ...
      "cancelReason": null,
      "cancelledAt": null,
      // ── Chỉ có giá trị khác null với type = ExchangeCurrency (7) ──────────
      "foreignAmount": null,     // số tiền nguồn khách đưa
      "targetCurrency": null,    // loại tiền trả khách
      "targetAmount": null,      // số tiền trả khách
      "exchangeRate": null,      // tỷ giá nguồn → LAK
      "targetRateToLak": null    // tỷ giá đích → LAK
    }
  ],
  "pagination": {
    "page": 1,
