...
export default function StockOutPage() {
  ...
  const rows = useMemo(() => (page?.data ?? []).filter(a => a.direction === 'OUT'), [page])
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      {isLoading ? (
        <TablePageSkeleton cols={5} rows={8} />
      ) : (
        <DataTable columns={columns} data={rows} hideSearch pageSize={20} />
      )}

      <InventoryBulkAdjustDialog open={createOpen} direction="OUT" branchId={effectiveBranchId} onClose={() => setCreateOpen(false)} />
    </div>
  )
}
