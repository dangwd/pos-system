...
import { Plus, X, PauseCircle, Copy, Search, LayoutDashboard, Sparkles, LogOut, ChevronDown } from 'lucide-react'
...
function ProductSearch({ products, onSelect }: ProductSearchProps) {
  ...
  return (
    <div ref={containerRef} className="relative shrink-0 w-64 xl:w-72">
      ...
      {open && (
        <div className="absolute top-full left-0 z-50 mt-1 w-80 rounded-lg border bg-popover shadow-xl overflow-hidden">
...
function InlineTabChip({ tab, isActive, showClose, onSwitch, onClose, onHold, onDuplicate }: InlineTabChipProps) {
  const t = useTranslations('pos.topBar')
  ...
  return (
    ...
    >
      {tab.status === 'holding' && (
        <span className="w-1.5 h-1.5 rounded-full bg-muted-foreground/70 shrink-0" title={t('holdingStatus')} />
      )}
      {tab.status === 'paying' && (
        <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse shrink-0" title={t('payingStatus')} />
      )}
  ...
  return (
    <Dialog open={!!tab} onOpenChange={o => !o && onCancel()}>
      <DialogContent className="max-w-sm">
        ...
        <DialogFooter>
          <Button variant="outline" onClick={onCancel}>{t('keep')}</Button>
          <Button variant="destructive" onClick={onConfirm}>{t('close')}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
...
export function PosTopBar({ products, onAddProduct }: PosTopBarProps) {
  ...
  return (
    <>
      ...
      <CloseConfirmDialog
        tab={pendingClose}
        onConfirm={() => { if (pendingClose) closeTab(pendingClose.id); setPendingClose(null) }}
