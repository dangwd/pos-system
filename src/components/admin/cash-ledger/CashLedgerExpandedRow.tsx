...
function Field({ label, value }: { label: string; value?: React.ReactNode }) {
  return (
    ...
    >
      <span
        style={{ minWidth: 120, fontSize: 12, color: "#6b7280", flexShrink: 0 }}
      >
        {label}:
      </span>
    ...
    >
      ...
      <div style={{ minWidth: 260, flex: 1 }}>
        ...
        {detail.referenceInvoiceCode && (
          <Field
            label={t("detail.referenceInvoice")}
            value={
        ...
        <Field
          ...
          value={
            <Tag
              color={SOURCE_COLOR[detail.source] ?? "default"}
        ...
        {detail.note && <Field label={t("detail.note")} value={detail.note} />}
      </div>
      ...
      <div style={{ minWidth: 260, flex: 1 }}>
        ...
        <Field
          ...
          value={t(`method.${detail.method}` as Parameters<typeof t>[0])}
        />
        {detail.cashAmountLak > 0 && (
          <Field
            label={t("detail.cashAmount")}
            value={formatKip(detail.cashAmountLak)}
          />
        )}
        {detail.bankAmountLak > 0 && (
          <Field
            label={t("detail.bankAmount")}
            value={formatKip(detail.bankAmountLak)}
          />
        )}
        {detail.currency !== "LAK" && (
          <Field
            label={t("detail.currency")}
            value={`${detail.currency} × ${detail.exchangeRate.toLocaleString("lo-LA")}`}
          />
        )}
